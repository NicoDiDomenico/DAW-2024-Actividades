document.addEventListener("DOMContentLoaded", function() {
    ...
    // Cargar datos desde LocalStorage si existen al cargar la página
    ...
    function validateForm() {
        ...
        if (!isValid) {
            showModal("modalError", "Errores en el formulario:<br>" + errorMessages.join("<br>"));
        } else {
            // Guardar datos en LocalStorage antes de enviar el formulario
            var formData = new FormData(form); // new FormData() recorre cada campo del formulario y recoge cada clave/valor conviertiendolos en un objeto
            var params = new URLSearchParams(formData).toString(); // new URLSearchParams() hace que cada clave: "valor" del objeto formulario pase al formato clave=valor. Esto va a ser útil para construir una URL para una solicitud GET
            var url = form.action + '?' + params; // Construir la URL completa con los parámetros de búsqueda
            
            showModal("modalSuccess", "<strong>Formulario enviado correctamente</strong><br>" + getFormData()); // Mostrar el modal de éxito

            window.location.href = url; // Redirigir el navegador a la URL construida
        }
    }

    // Función para cargar datos desde LocalStorage
    function loadFormData() {
        var storedData = localStorage.getItem('formData'); // Obtener los datos almacenados bajo la clave 'formData' en LocalStorage
        if (storedData) { // Verificar si hay datos almacenados
               var data = JSON.parse(storedData); // JSON.parse es un método que toma una cadena de texto en formato JSON y la convierte en un objeto de JavaScript.
            inputs.forEach(function(input) {
                if (data[input.name]) {
